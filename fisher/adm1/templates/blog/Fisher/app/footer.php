    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-lg-12">
        <div class="logo">
          <div class="wrapp">
            <div class="img_wrapp">
              <img src="{{  footer.logo34x46  }}" alt="Logo">
            </div>
            ...
            <ul>
              <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                  if ( !file_exists('img/icons/'.$row['link_image_dark']) or empty($row['link_image_dark']) ) {
                    $row['link_image_dark'] = 'img/items/template.png';
                    $row['name_alt']   =  'Изображение временно отсутствует';
                  } else {
                    $row['link_image_dark'] = 'img/icons/'.$row['link_image_dark'];
                    $row['name_alt']   = $row['name'];
                  }
                  ?>
                    <li>
                      <a href="<?php print($row['link']); ?>">
                       <img src="<?php print($row['link_image_dark']); ?>" alt="<?php print($row['name_alt']); ?>">
                      </a>
                    </li>
                  <?php
                }
          ...
          <div class="wrapp">
            ...
            <div class="content">
              <a href="tel:<?php 
                  ...
                  print($phone);
                ?>">
                Позвоните нам: {{  contacts.number  }}</a> 
              </div>
          </div>
        </div>
      </div>
